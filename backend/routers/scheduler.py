...
async def create_schedule(req: CreateScheduleRequest):
    ...
    try:
        ...
        # C. Save to DB (Record the ARN)
        # Check if exists first to update or insert
        current_schedule = await db_fetchrow(
            pool,
            "SELECT * FROM study_schedules WHERE user_id = $1 AND book_id = $2",
        ...
        if current_schedule:
            await db_execute(
                ...
                """UPDATE study_schedules
                   SET telegram_chat_id = $1, cron_schedule = $2, qstash_schedule_id_3 = $3
                   WHERE id = $4""",
                target_chat_id,
                f"{req.hour}:{req.minute} {req.timezone}",
                response.get('ScheduleArn'),
                current_schedule['id']
            )
        else:
            await db_execute(
                pool,
                """INSERT INTO study_schedules (user_id, book_id, telegram_chat_id, cron_schedule, qstash_schedule_id_3)
                   VALUES ($1, $2, $3, $4, $5)""",
                req.userId, req.bookId, target_chat_id,
                f"{req.hour}:{req.minute} {req.timezone}",
                response.get('ScheduleArn')
            )
...
async def delete_schedule(user_id: str, book_id: str):
    ...
    try:
        ...
        await db_execute(
            pool,
            "DELETE FROM study_schedules WHERE user_id = $1 AND book_id = $2",
            user_id, book_id
        )
    ...
    if payload.type == "5min":
        try:
            session_row = await db_fetchrow(
                pool,
                """INSERT INTO study_sessions (user_id, book_id, status, notification_type)
                   VALUES ($1, $2, $3, $4) RETURNING *""",
                payload.userId, payload.bookId, "pending", "5min"
            )
