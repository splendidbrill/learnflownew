...
import { vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism'; // Code CSS
...
interface Paragraph {
  ...
  explanation?: string; // Stores the AI analysis for images
}
...
export const BookClient: React.FC<BookClientProps> = ({ bookId }) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "I'm having trouble connecting. Try clicking 'Study Section' again.",
        },
      ]);
    } finally {
      ...
      <div className="max-w-5xl mx-auto pb-20">
        <div className="text-center mb-12">
          ...
          <div className="flex justify-center mt-4">
            <span
              className={`px-4 py-1.5 rounded-full text-xs font-bold tracking-wider border uppercase ${
        ...
        {bookStatus === "processing" && (
          <div className="text-center py-20 border-2 border-dashed border-blue-500/30 rounded-3xl bg-blue-500/5 animate-pulse">
            ...
            <p className="text-blue-200/60 text-lg">
              Our AI is reading the Table of Contents.
            </p>
          </div>
        )}
        ...
        {bookStatus === "pending" && (
          <div className="text-center py-24 border-2 border-dashed border-white/10 rounded-3xl bg-white/5">
            ...
            >
              {isGenerating ? (
                <>
                  <Loader2 className="w-6 h-6 animate-spin mr-3" /> Starting AI
                  Engine...
                </>
              ) : (
                <>
        ...
        {bookStatus === "failed" && (
          <div className="text-center py-12 bg-red-900/10 border border-red-500/20 rounded-2xl">
            ...
            <button
              ...
              className="px-6 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-lg text-sm font-medium transition-colors mt-4"
            >
              Reset & Try Again
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
            {chapters.map((chapter) => (
              ...
              >
                ...
                <h3 className="text-lg font-bold text-white mb-2 line-clamp-2 leading-snug group-hover:text-purple-200">
                  {chapter.title}
                </h3>
              </button>
            ))}
    ...
    if (!nextPara) {
      setMessages((prev) => [
        ...prev,
        {
          id: Date.now().toString(),
          role: "assistant",
          content: `You've already finished **${section.title}**. Do you have any specific questions about it?`,
        },
      ]);
      setActiveParagraphId(null);
    } else {
  ...
  const handleNextParagraph = async () => {
    ...
    if (nextPara) {
      ...
      if (currentSectionTitle !== nextSectionTitle) {
        ...
        const newSectionParas = paragraphs.filter(
          (p) => (p.section_title || "General") === nextSectionTitle
        );
        ...
        addMessageToDb(
          "assistant",
          `--- Starting: ${nextSectionTitle} (0/${total}) ---`
        );
      }
      // -----------------------------
  ...
  const sectionParas = paragraphs.filter(
    (p) => (p.section_title || "General") === currentSectionTitle
  );
  const completedInThisSection = sectionParas.filter(
    (p) => p.is_completed
  ).length;
  ...
  //               The AI hasn't read this chapter yet. Click below to generate the
  //               learning modules.
  //             </p>
  ...
  //                           />
  //                         </div>
  //                         {/* We will hook up the "Explain" button later for Vision AI */}
  //                         <button className="mt-2 text-xs text-blue-400 flex items-center gap-1">
  ...
  //                       <div
  //                         key={para.id}
  //                         className="my-4 rounded-lg overflow-hidden border border-white/10 shadow-lg text-sm"
  //                       >
  //                         <SyntaxHighlighter
  //                           language="python" // We assume Python/Generic for now
  //                           style={vscDarkPlus}
  //                           customStyle={{
  //                             margin: 0,
  //                             padding: "1.5rem",
  ...
  //                   // 3. TEXT/MATH RENDERER
  //                   return (
  //                     <div
  //                       key={para.id}
  //                       className={`
  //       text-gray-300 leading-relaxed p-4 rounded-lg transition-all
  //       ${
  //         para.is_completed
  //           ? "border-b-2 border-dotted border-orange-500 text-gray-400"
  //           : "bg-white/5"
  //       }
  //       ${
  //         activeParagraphId === para.id
            ...
            >
              ...
              <div className="space-y-6">
                {section.paragraphs.map((para) => {
                  ...
                  if (para.type === 'image') {
                    return (
                      <div key={para.id} className={`flex flex-col items-center p-4 rounded-xl ${activeParagraphId === para.id ? 'bg-purple-600/10 ring-1 ring-purple-500' : ''}`}>
                        <div className="bg-white/5 p-2 rounded-xl border border-white/10 overflow-hidden">
                           <img src={para.content} alt="Diagram" className="max-w-full max-h-[400px] object-contain rounded-lg" />
                        </div>
                        
                        {para.explanation ? (
                           <div className="mt-4 w-full bg-blue-900/20 border-l-4 border-cyan-400 p-4 rounded-r-lg text-sm text-gray-200 animate-in fade-in">
                              <strong className="text-cyan-400 block mb-1 uppercase text-xs tracking-wider">AI Vision Analysis</strong>
                              {para.explanation}
                           </div>
                        ) : (
                           <button 
                             ...
                             className="mt-3 px-4 py-2 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 rounded-full text-xs font-bold text-blue-300 flex items-center gap-2 transition-all"
                           >
                  ...
                  if (para.type === 'code') {
                    return (
                      <div key={para.id} className={`rounded-lg overflow-hidden border border-white/10 shadow-lg text-sm my-4 ${activeParagraphId === para.id ? 'ring-2 ring-purple-500' : ''}`}>
                        <SyntaxHighlighter 
                          language="python" 
                          style={vscDarkPlus}
                          customStyle={{ margin: 0, padding: '1.5rem', background: '#0a0212' }}
                          wrapLongLines={true}
                        >
                          {para.content}
                        </SyntaxHighlighter>
  ...
  if (isLoadingData)
    return (
      <div className="bg-[#13002b] h-screen text-white flex items-center justify-center">
  ...
  return (
    <div className="flex h-screen bg-[#13002b] overflow-hidden font-sans">
      {/* LEFT SIDEBAR */}
      <div
        className={`flex-shrink-0 bg-[#0a0212] border-r border-white/5 transition-all duration-300 flex flex-col ${
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1">
          {chapters.map((chapter) => (
            ...
            >
              <span className="flex-shrink-0 w-6 h-6 rounded bg-white/5 flex items-center justify-center text-xs font-mono">
                {chapter.order_index}
              </span>
              <span className="truncate flex-1">{chapter.title}</span>
              {chapter.start_page_num > 0 && (
                <span className="text-[10px] text-gray-600">
                  {chapter.start_page_num}
                </span>
              )}
            </button>
          ))}
            ...
            <div className="mt-3 animate-in fade-in slide-in-from-top-2">
              ...
              <div className="h-1.5 w-full bg-black/40 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${
                      (completedInThisSection / (sectionParas.length || 1)) *
                      100
                    }%`,
                  }}
                />
